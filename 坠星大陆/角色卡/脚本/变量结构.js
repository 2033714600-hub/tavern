...
export const Schema = z.object({
  世界: z.object({
    ...
    研发进度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
  }).prefault({}),

  营地: z.object({
    人口: z.coerce.number().transform(v => _.clamp(v, 0, 9999)).prefault(14),
    生存指标: z.object({
      温饱度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(15),
      栖居度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(10),
      舒适度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
    }).prefault({}),
    营地状态: z.object({
    ...
    向心力: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(5),
    篝火燃料: z.object({
      当前: z.coerce.number().prefault(0),
      上限: z.coerce.number().prefault(1000),
    }).prefault({}),
    防御等级: z.coerce.number().transform(v => _.clamp(v, 1, 5)).prefault(1),
    防御工事: z.object({
      耐久: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(85),
    ...
    仓库等级: z.coerce.number().transform(v => _.clamp(v, 1, 5)).prefault(1),
    仓库设施: z.object({
    ...
    无名族人基准效率: z.coerce.number().transform(v => _.clamp(v, 20, 200)).prefault(40),
    工作队列并行上限: z.coerce.number().transform(v => _.clamp(v, 1, 20)).prefault(3),
    当前建筑: z.record(
  ...
  部族事务: z.record(
    ...
    '熬过第一个寒夜': { 说明: '必须尽快生火并分配有限的食物，否则向心力会持续下降。', 级别: '主要' },
    '评估残骸价值': { 说明: '兽耳娘们还不知道这个铁壳子能换来什么，需要首领做出判断。', 级别: '次要' },
  }),
  ...
  探索编队: z.record(
    ...
    z.object({
      ...
      计划时长: z.coerce.number().transform(v => _.clamp(v, 2, 8)).prefault(4),
  ...
  探索区域: z.record(
    ...
    z.object({
      ...
      探索度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
    }),
  ).prefault({
    '飞船残骸周边': {
  ...
  工作队列: z.record(
    ...
    z.object({
      ...
      效率加成: z.coerce.number().transform(v => _.clamp(v, 0, 80)).prefault(0),
      负责人: z.string().prefault(''),
      所需工时: z.coerce.number().prefault(0),
      优先级: z.coerce.number().transform(v => _.clamp(v, 1, 9)).prefault(5),
    }),
  ).prefault({}),
  ...
  具名NPC: z.record(
    ...
    z.object({
      ...
      忠诚度: z.coerce.number().transform(v => _.clamp(v, -100, 100)).prefault(0),
      ...
      饱食度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(40),
      ...
      技能: z.object({
        狩猎: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
        战斗: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
        采集: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
        后勤: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
      }).prefault({}),
    }),
  ).prefault({}),
  ...
  主角: z.object({
    ...
    威望: z.coerce.number().transform(v => _.clamp(v, 0, 9999)).prefault(0),
    随身装备: z.record(
